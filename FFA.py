...
def FFA(objf, lb, ub, dim, n, MaxGeneration):
    ...
    for k in range(0, MaxGeneration):  # start iterations
        ...
        if k % 1 == 0:
            print(["At iteration " + str(k) + " the best fitness is " + str(BestQuality) + "alpha fetures is" + str(featurecount)])
            print('alpha:', numpy.where( nbest>0.5)[0]);
            print(["At iteration " + str(k) + " the best fitness is " + str(BestQuality) + "betamin fetures is" + str(featurecount)])
            print('betamin:', numpy.where( nbest>0.20)[0]);
